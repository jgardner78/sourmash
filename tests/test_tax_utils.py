...
def test_check_and_load_gather_csvs_empty(runtmp):
    ...
    with pytest.raises(Exception) as exc:
        gather_results, ids_missing, n_missing, header = check_and_load_gather_csvs(
            csvs, tax_assign
        )
...
def test_check_and_load_gather_csvs_with_empty_force(runtmp):
    ...
    g_results = [x.replace("GCF_001881345.1", "gA") for x in open(g_csv)]
...
def test_summarize_gather_at_1():
    ...
    taxD = make_mini_taxonomy([gA_tax, gB_tax])
    # run summarize_gather_at and check results!
    sk_sum, _, _ = summarize_gather_at(
        "superkingdom", taxD, g_res, estimate_query_ani=True
    )
...
def test_summarize_gather_at_over100percent_f_unique_to_query():
    ...
    with pytest.raises(ValueError) as exc:
        sk_sum, _, _ = summarize_gather_at("superkingdom", taxD, g_res)
    assert (
        "The tax summary of query 'queryA' is 1.1, which is > 100% of the query!!"
        in str(exc)
    )
...
def test_summarize_gather_at_best_only_0():
    ...
    # run summarize_gather_at and check results!
    sk_sum, _, _ = summarize_gather_at(
        "superkingdom", taxD, g_res, best_only=True, estimate_query_ani=True
    )
    # superkingdom
    assert len(sk_sum) == 1
    print("superkingdom summarized gather: ", sk_sum[0])
    ...
    # phylum
    phy_sum, _, _ = summarize_gather_at(
        "phylum", taxD, g_res, best_only=True, estimate_query_ani=True
    )
    print("phylum summarized gather: ", phy_sum[0])
    assert len(phy_sum) == 1
    assert phy_sum[0].lineage == (
    ...
    cl_sum, _, _ = summarize_gather_at(
        "class", taxD, g_res, best_only=True, estimate_query_ani=True
    )
    assert len(cl_sum) == 1
    print("class summarized gather: ", cl_sum)
    assert cl_sum[0].lineage == (
...
def test_write_summary_csv(runtmp):
    ...
    sum_gather = {
        "superkingdom": [
            SummarizedGatherResult(
                ...
                rank="superkingdom",
                fraction=1.0,
                query_md5="queryA_md5",
                query_filename="queryA.sig",
    ...
    sr = [x.rstrip().split(",") for x in open(outs)]
...
def test_aggregate_by_lineage_at_rank_by_query():
    ...
    sk_lin_sum, query_names, num_queries = aggregate_by_lineage_at_rank(
        sk_sum, by_query=True
    )
    print("superkingdom lineage summary:", sk_lin_sum, "\n")
    assert sk_lin_sum == {
        (LineagePair(rank="superkingdom", name="a"),): {"queryA": 0.9, "queryB": 0.3},
    ...
    phy_lin_sum, query_names, num_queries = aggregate_by_lineage_at_rank(
        phy_sum, by_query=True
    )
...
def test_write_krona(runtmp):
    ...
    kr = [x.strip().split("\t") for x in open(outk)]
...
def test_combine_sumgather_csvs_by_lineage(runtmp):
    ...
    sum_gather2 = {
        "superkingdom": [
            SummarizedGatherResult(
                ...
                rank="superkingdom",
                fraction=0.7,
                query_md5="queryB_md5",
                query_filename="queryB.sig",
    ...
    assert query_names == ["queryA", "queryB"]
    linD, query_names = combine_sumgather_csvs_by_lineage(
        [sg1, sg2], rank="superkingdom"
    )
    print("lineage dict: \n", linD)
...
def test_write_lineage_sample_frac(runtmp):
    ...
    frac_lines = [x.strip().split("\t") for x in open(outfrac)]
...
def test_combine_sumgather_csvs_by_lineage_improper_rank(runtmp):
    ...
    # test combine_summarized_gather_csvs_by_lineage_at_rank
    with pytest.raises(ValueError) as exc:
        linD, sample_names = combine_sumgather_csvs_by_lineage(
            [sg1, sg2], rank="strain"
        )

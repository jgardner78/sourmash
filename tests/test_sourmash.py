...
def test_multigather_metagenome_query_with_sbt_addl_query(c):
    ...
    cmd = f"multigather --query {another_query} gcf_all.sbt.zip --db gcf_all.sbt.zip -k 21 --threshold-bp=0"
    cmd = cmd.split(" ")
    c.run_sourmash(*cmd)
...
def test_multigather_metagenome_sbt_query_from_file_with_addl_query(c):
    ...
    cmd = f"multigather --query {another_query} --query-from-file {query_list} --db gcf_all.sbt.zip -k 21 --threshold-bp=0"
    cmd = cmd.split(" ")
    c.run_sourmash(*cmd)

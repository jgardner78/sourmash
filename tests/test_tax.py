...
def test_metagenome_human_format_out(runtmp):
    ...
    assert (
        outp[3]
        == "test1              5.8%     d__Bacteria;p__Proteobacteria;c__Gammaproteobacteria;o__Enterobacterales;f__Enterobacteriaceae;g__Escherichia"
    )
    assert (
        outp[4]
        == "test1              5.7%     d__Bacteria;p__Bacteroidota;c__Bacteroidia;o__Bacteroidales;f__Bacteroidaceae;g__Prevotella"
    )
    assert (
        outp[5]
        == "test1              1.6%     d__Bacteria;p__Bacteroidota;c__Bacteroidia;o__Bacteroidales;f__Bacteroidaceae;g__Phocaeicola"
    )
...
def test_metagenome_no_rank_lineage_summary(runtmp):
    ...
    assert (
        "Rank (--rank) is required for krona and lineage_summary output formats."
        in str(exc.value)
    )
...
def test_metagenome_duplicated_taxonomy_fail(runtmp):
    ...
    with open(duplicated_csv, "w") as dup:
        tax = [x.rstrip() for x in open(taxonomy_csv)]
...
def test_metagenome_missing_fail_taxonomy(runtmp):
    ...
    assert (
        "Failing on missing taxonomy, as requested via --fail-on-missing-taxonomy."
        in str(exc.value)
    )
    assert c.last_result.status == -1
...
def test_metagenome_bad_gather_header(runtmp):
    ...
    bad_g = [x.replace("name", "nope") for x in open(g_csv)]
    ...
    assert f"Not all required gather columns are present in '{bad_g_csv}'." in str(
        exc.value
    )
    assert runtmp.last_result.status == -1
...
def test_metagenome_over100percent_error(runtmp):
    ...
    assert runtmp.last_result.status == -1
    assert (
        "ERROR: The tax summary of query 'test1' is 1.1160749900279219, which is > 100% of the query!!"
        in runtmp.last_result.err
    )
...
def test_genome_bad_gather_header(runtmp):
    ...
    # creates bad gather result
    bad_g = [x.replace("f_unique_to_query", "nope") for x in open(g_csv)]
    with open(bad_g_csv, "w") as fp:
        for line in bad_g:
            fp.write(line)
    print("bad_gather_results: \n", bad_g)
...
def test_genome_rank_stdout_0(runtmp):
    ...
    assert (
        "test1,match,species,0.089,d__Bacteria;p__Bacteroidota;c__Bacteroidia;o__Bacteroidales;f__Bacteroidaceae;g__Prevotella;s__Prevotella copri,md5,test1.sig,0.057,444000.0"
        in c.last_result.out
    )
...
def test_genome_rank_stdout_0_db(runtmp):
    ...
    assert c.last_result.status == 0
    assert (
        "WARNING: classifying query test1 at desired rank species does not meet containment threshold 1.0"
        in c.last_result.err
    )
    assert (
        "test1,below_threshold,species,0.089,d__Bacteria;p__Bacteroidota;c__Bacteroidia;o__Bacteroidales;f__Bacteroidaceae;g__Prevotella;s__Prevotella copri,md5,test1.sig,0.057,444000.0,"
        in c.last_result.out
    )
...
def test_genome_rank_csv_0(runtmp):
    ...
    assert (
        "test1,match,species,0.0885520542481053,d__Bacteria;p__Bacteroidota;c__Bacteroidia;o__Bacteroidales;f__Bacteroidaceae;g__Prevotella;s__Prevotella copri,md5,test1.sig,0.05701254275940707,444000.0"
        in cl_results[1]
    )
...
def test_genome_rank_human_output(runtmp):
    ...
    assert (
        outp[2]
        == "test1              5.7%     d__Bacteria;p__Bacteroidota;c__Bacteroidia;o__Bacteroidales;f__Bacteroidaceae;g__Prevotella;s__Prevotella copri"
    )
...
def test_genome_gather_two_files(runtmp):
    ...
    g_res2 = runtmp.output("test2.gather.csv")
    test2_results = [x.replace("test1", "test2") for x in open(g_res)]
    with open(g_res2, "w") as fp:
    ...
    assert (
        "test2,match,species,0.089,d__Bacteria;p__Bacteroidota;c__Bacteroidia;o__Bacteroidales;f__Bacteroidaceae;g__Prevotella;s__Prevotella copri,md5,test2.sig,0.057,444000.0"
        in c.last_result.out
    )
...
def test_genome_gather_two_queries(runtmp):
    ...
    q2_results = [x for x in open(g_res)]
    # rename queries
...
def test_genome_multiple_taxonomy_files(runtmp):
    ...
    assert (
        "test1,match,family,0.116,d__Bacteria;p__Bacteroidota;c__Bacteroidia;o__Bacteroidales;f__Bacteroidaceae,md5,test1.sig,0.073,582000.0,"
        in c.last_result.out
    )
    # using single -t arg
    c.run_sourmash(
...
def test_genome_empty_gather_results_with_header_single(runtmp):
    ...
    g_csv = utils.get_test_data("tax/test1.gather.csv")
    gather_results = [x for x in open(g_csv)]
...
def test_genome_over100percent_error(runtmp):
    ...
    assert runtmp.last_result.status == -1
    assert (
        "ERROR: The tax summary of query 'test1' is 1.1, which is > 100% of the query!!"
        in runtmp.last_result.err
    )
...
def test_genome_ani_threshold(runtmp):
    ...
    c.run_sourmash(
        "tax", "genome", "-g", g_csv, "--taxonomy-csv", tax, "--ani-threshold", "0.95"
    )
    ...
    assert c.last_result.status == 0
    assert (
        "WARNING: Please run gather with sourmash >= 4.4 to estimate query ANI at rank. Continuing without ANI..."
        not in c.last_result.err
    )
    assert (
        "query_name,status,rank,fraction,lineage,query_md5,query_filename,f_weighted_at_rank,bp_match_at_rank"
    ...
    assert (
        "test1,match,family,0.116,d__Bacteria;p__Bacteroidota;c__Bacteroidia;o__Bacteroidales;f__Bacteroidaceae,md5,test1.sig,0.073,582000.0,0.9328896594471843"
        in c.last_result.out
    )
    ...
    print(c.last_result.err)
    assert (
        "WARNING: classifying query test1 at desired rank species does not meet query ANI/AAI threshold 1.0"
        in c.last_result.err
    )
    assert (
        "test1,below_threshold,species,0.089,d__Bacteria;p__Bacteroidota;c__Bacteroidia;o__Bacteroidales;f__Bacteroidaceae;g__Prevotella;s__Prevotella copri,md5,test1.sig,0.057,444000.0,0.9247805047263588"
        in c.last_result.out
    )
...
def test_annotate_bad_gather_header(runtmp):
    ...
    # creates bad gather result
    bad_g = [x.replace("query_name", "nope") for x in open(g_csv)]
    with open(bad_g_csv, "w") as fp:
        for line in bad_g:
            fp.write(line)
    print("bad_gather_results: \n", bad_g)

...
QueryInfo = namedtuple(
    "QueryInfo",
    "query_md5, query_filename, query_bp, query_hashes, total_weighted_hashes",
)
SummarizedGatherResult = namedtuple(
    ...
    *,
    ...
    with sourmash_args.FileInputCSV(gather_csv) as r:
        ...
        if not set(essential_colnames).issubset(header):
            raise ValueError(
                f"Not all required gather columns are present in '{gather_csv}'."
            )
        ...
        for n, row in enumerate(r):
            ...
            if query_name in seen_queries:
                ...
                raise ValueError(
                    f"Gather query {query_name} was found in more than one CSV. Cannot load from '{gather_csv}'."
                )
            else:
...
def check_and_load_gather_csvs(
    gather_csvs, tax_assign, *, fail_on_missing_taxonomy=False, force=False
):
    """
    ...
    for n, gather_csv in enumerate(gather_csvs):
        these_results = []
        try:
            these_results, header, seen_queries = load_gather_results(
                gather_csv, seen_queries=seen_queries, force=force
            )
    ...
    notify(
        f"loaded {len(gather_results)} results total from {str(num_gather_csvs_loaded)} gather CSVs"
    )

    return gather_results, all_ident_missed, total_missed, header


def find_match_lineage(
    match_ident,
    tax_assign,
    *,
    skip_idents=[],
...
def summarize_gather_at(
    ...
    *,
    ...
    for row in gather_results:
        ...
        # get query_bp
        if (
            query_name not in query_info.keys()
        ):  # REMOVING THIS AFFECTS GATHER RESULTS!!! BUT query bp should always be same for same query? bug?
        ...
        # store query info
        query_info[query_name] = QueryInfo(
            query_md5=query_md5,
            query_filename=query_filename,
            query_bp=query_bp,
            query_hashes=query_nhashes,
        ...
        if (
            f_unique_to_query >= 1.0 and query_name not in seen_perfect
        ):  # only want to notify once, not for each rank
            ident = get_ident(
                match_ident,
                keep_full_identifiers=keep_full_identifiers,
                keep_identifier_versions=keep_identifier_versions,
            ...
            notify(
                f'WARNING: 100% match! Is query "{query_name}" identical to its database match, {ident}?'
            )

        # get lineage for match
        lineage = find_match_lineage(
            match_ident,
            tax_assign,
    ...
    for query_name, lineage_weights in sum_uniq_to_query.items():
        ...
        if best_only:
            ...
            if fraction > 1:
                raise ValueError(
                    f"The tax summary of query '{query_name}' is {fraction}, which is > 100% of the query!! This should not be possible. Please check that your input files come directly from a single gather run per query."
                )
            elif fraction == 0:
                continue
            ...
            if estimate_query_ani:
                query_ani = containment_to_distance(
                    fraction,
                    ksize,
                    scaled,
                    n_unique_kmers=qInfo.query_hashes,
                    sequence_len_bp=qInfo.query_bp,
                ).ani
            sres = SummarizedGatherResult(
                ...
                lineage,
                qInfo.query_md5,
                qInfo.query_filename,
                f_weighted_at_rank,
                bp_intersect_at_rank,
                query_ani,
                qInfo.total_weighted_hashes * scaled,
            )
...
def find_missing_identities(gather_results, tax_assign):
    ...
    if ident_missed:
        notify(
            f"of {len(gather_results)} gather results, missed {len(ident_missed)} lineage assignments."
        )
    return ident_missed
...
def format_for_krona(rank, summarized_gather):
    ...
    for res_rank, rank_results in summarized_gather.items():
        if res_rank == rank:
            lineage_summary, all_queries, num_queries = aggregate_by_lineage_at_rank(
                rank_results, by_query=False
            )
...
def write_kreport(summarized_gather, csv_fp, *, sep="\t"):
    ...
    for rank, rank_results in summarized_gather.items():
        ...
        for res in rank_results:
            ...
            # weighted bp
            kresD["num_bp_contained"] = int(
                res.f_weighted_at_rank * res.total_weighted_hashes
            )
...
def write_human_summary(summarized_gather, out_fp, display_rank):
    ...
    if found_ANI:
        ...
        for rD in results:
            out_fp.write(
                "{query_name:<15s}   {f_weighted_at_rank}     {lineage}\n".format(**rD)
            )
...
def write_classifications(
    classifications, csv_fp, *, sep=",", limit_float_decimals=False
):
    """
...
def combine_sumgather_csvs_by_lineage(
    gather_csvs,
    *,
    rank="species",
    accept_ranks=list(lca_utils.taxlist(include_strain=False)),
    force=False,
):
    """
...
def write_lineage_sample_frac(
    sample_names, lineage_dict, out_fp, *, format_lineage=False, sep="\t"
):
    """
        ...
        with sourmash_args.FileInputCSV(filename) as r:
            ...
            if "ident" not in header:
                ...
                if "identifiers" in header:
                    ...
                    return cls.load_from_gather_with_lineages(filename, force=force)
                else:
    ...
    def load_from_gather_with_lineages(cls, filename, *, force=False):
        ...
        with sourmash_args.FileInputCSV(filename) as r:
            ...
            if "name" not in header or "lineage" not in header:
                raise ValueError(
                    f"Expected headers 'name' and 'lineage' not found. Is this a with-lineages file?"
                )

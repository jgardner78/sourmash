...
def split(args):
    ...
    output_scaled_template = (
        "{md5sum}.k={ksize}.scaled={scaled}.{moltype}.dup={dup}.{basename}.sig"
    )
    output_num_template = (
        "{md5sum}.k={ksize}.num={num}.{moltype}.dup={dup}.{basename}.sig"
    )
...
def collect(args):
    ...
    for n_files, loc in enumerate(args.locations):
        ...
        if idx.manifest is None and require_manifest:
            ...
            error(
                f"sig collect requires a manifest by default, but no manifest present."
            )
            error("specify --no-require-manifest to dynamically generate one.")
            sys.exit(-1)
